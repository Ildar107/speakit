...
const WordItem = (props) => {
    return (
      <div 
        className={`game__item btn btn-outline-info ${props.isSpeakMode ? 'game__item_disabled': ''} ${props.isKnown ? 'game__item_active': ''}`} 
        onClick={props.trainingClick}
        ...
        <audio src={`https://raw.githubusercontent.com/irinainina/rslang-data/master/data/${props.audio.replace('files/', '')}`} crossOrigin="anonymous">
        </audio>
        <p className="word"> {props.word}</p>
        <p className="transcription"> {props.transcription}</p>
        {/* <p className="translate" style={display: 'none'}></p> */}
      </div>
    );
}
