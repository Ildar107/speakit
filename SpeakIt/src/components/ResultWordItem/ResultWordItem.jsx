...
class ResultWordItem extends Component {
    constructor(props) {
        super(props);
        this.state = {
            translate: '',
    ...
    render = () => {
        return (
            <div 
                ...
                <audio src={`https://raw.githubusercontent.com/irinainina/rslang-data/master/data/${this.props.audio.replace('files/', '')}`} crossOrigin="anonymous">
                </audio>
                <p className="result__word"> {this.props.word}</p>
                <p className="result__transcription"> {this.props.transcription}</p>
                <p className="result__translate"> {this.state.translate}</p>
            </div>
        );
    }
}
export default ResultWordItem;
